...
GREEN='\033[0;32m'
NC='\033[0m'                                # No Color
F1="./other"                                #folder not copied  
F2="./COFTABL${CDB}.ISI"                         #files not copied
F3="./COFTABL${CDB}.TXT"                         #files not copied
###############################################################################
# set array
###############################################################################
...
echo "************************************************************************"
echo "- Copying files..."
for i in ${array[*]}; do            #copy all files
  cp -f $ZDIR"/"$i ./  2>/dev/null  #copy file and pipe output to null
  ((COUNT++))                       #increment count
  if [ -e "./"$i ]; then            #if file was copied here...
    ((CPCNT++))                     #increment count
  else                              #wasn't copied
...
echo -e ${RED}"Excluded files (not copied):"${NC}
echo -e ${RED}"-->"${NC} $F1
echo -e ${RED}"-->"${NC} $F2
echo -e ${RED}"-->"${NC} $F3
echo "************************************************************************"
